...
#include "common/third_party/hpipm_cpp/interface/ocp_qp_ipm_solver.h"
...
//   WrapperHolder() 
//     : ocp_qp_dim_wrapper(std::make_shared<DOcpQpDimWrapper>()),
//       ocp_qp_ipm_arg_wrapper(std::make_shared<DOcpQpIpmArgWrapper>()),
//       ocp_qp_wrapper(),
//       ocp_qp_sol_wrapper(),
//       ocp_qp_ipm_ws_wrapper() {}
//   ~WrapperHolder() = default;

//   std::shared_ptr<DOcpQpDimWrapper> ocp_qp_dim_wrapper;
//   std::shared_ptr<DOcpQpIpmArgWrapper> ocp_qp_ipm_arg_wrapper;
//   DOcpQPWrapper ocp_qp_wrapper;
//   DOcpQpSolWrapper ocp_qp_sol_wrapper;
//   DOcpQpIpmWsWrapper ocp_qp_ipm_ws_wrapper;
// };
...
//   set_solver_settings(solver_settings);
//   resize(ocp_qp);
// }
...
void OcpQpIpmSolver::set_solver_settings(
    const OcpQpIpmSolverSettings& solver_settings) {
  // d_ocp_qp_ipm_arg* ocp_qp_ipm_arg_ptr = 
  //   wrapper_holder_->ocp_qp_ipm_arg_wrapper->get();
...
void OcpQpIpmSolver::resize(const std::vector<OcpQp>& ocp_qp) {
  ...
  // wrapper_holder_->ocp_qp_ipm_arg_wrapper->resize(wrapper_holder_->ocp_qp_dim_wrapper);
  // d_ocp_qp_ipm_arg* ocp_qp_ipm_arg_ptr = 
  //   wrapper_holder_->ocp_qp_ipm_arg_wrapper->get();
  // auto convertHpipmMode = [](const HpipmMode mode) {
  //   switch (mode) {
...
HpipmStatus OcpQpIpmSolver::solve(
...
************************************************/
	...
	d_ocp_qp_set_all(A_ptr_.data(), B_ptr_.data(), b_ptr_.data(), 
                   ...
                   idxs_ptr_.data(), lls_ptr_.data(), lus_ptr_.data(), &qp);
...
************************************************/
  ...
  auto hpipm_mode = convertHpipmMode(solver_settings_.mode);
 	...
 	d_ocp_qp_ipm_arg_set_default(hpipm_mode, &arg);

	d_ocp_qp_ipm_arg_set_mu0(&solver_settings_.mu0, &arg);
	d_ocp_qp_ipm_arg_set_iter_max(&solver_settings_.iter_max, &arg);
	d_ocp_qp_ipm_arg_set_alpha_min(&solver_settings_.alpha_min, &arg);
	d_ocp_qp_ipm_arg_set_mu0(&solver_settings_.mu0, &arg);
	d_ocp_qp_ipm_arg_set_tol_stat(&solver_settings_.tol_stat, &arg);
	d_ocp_qp_ipm_arg_set_tol_eq(&solver_settings_.tol_eq, &arg);
	d_ocp_qp_ipm_arg_set_tol_ineq(&solver_settings_.tol_ineq, &arg);
	d_ocp_qp_ipm_arg_set_tol_comp(&solver_settings_.tol_comp, &arg);
	d_ocp_qp_ipm_arg_set_reg_prim(&solver_settings_.reg_prim, &arg);
	d_ocp_qp_ipm_arg_set_warm_start(&solver_settings_.warm_start, &arg);
	d_ocp_qp_ipm_arg_set_pred_corr(&solver_settings_.pred_corr, &arg);
	d_ocp_qp_ipm_arg_set_ric_alg(&solver_settings_.ric_alg, &arg);
  d_ocp_qp_ipm_arg_set_split_step(&solver_settings_.split_step, &arg);
...
************************************************/
  if (solver_settings_.warm_start) {
    for (int i = 0; i < dim_.N; ++i) {
      d_ocp_qp_sol_set_x(i, solution[i].x.data(), &qp_sol);
      d_ocp_qp_sol_set_u(i, solution[i].u.data(), &qp_sol);
    }
    d_ocp_qp_sol_set_x(dim_.N, solution[dim_.N].x.data(), &qp_sol);
  }

	d_ocp_qp_ipm_solve(&qp, &qp_sol, &arg, &workspace);

  for (int i = 0; i < dim_.N; ++i) {
    d_ocp_qp_sol_get_x(i, &qp_sol, solution[i].x.data());
    d_ocp_qp_sol_get_u(i, &qp_sol, solution[i].u.data());
  }
  d_ocp_qp_sol_get_x(dim_.N, &qp_sol, solution[dim_.N].x.data());
  
  int hpipm_status;
	d_ocp_qp_ipm_get_status(&workspace, &hpipm_status);

  // get solver statistics
  d_ocp_qp_ipm_get_iter(&workspace, &solver_statistics_.iter);
  d_ocp_qp_ipm_get_max_res_stat(&workspace, &solver_statistics_.max_res_stat);
  d_ocp_qp_ipm_get_max_res_eq(&workspace, &solver_statistics_.max_res_eq);
  d_ocp_qp_ipm_get_max_res_ineq(&workspace, &solver_statistics_.max_res_ineq);
  d_ocp_qp_ipm_get_max_res_comp(&workspace, &solver_statistics_.max_res_comp);

  const int stat_m = 18;
  solver_statistics_.clear();
  solver_statistics_.reserve(solver_statistics_.iter + 2);
  for (int i = 0; i <= solver_statistics_.iter + 1; ++i) {
    solver_statistics_.alpha_aff.push_back(workspace.stat[stat_m * i + 0]);
    solver_statistics_.mu_aff.push_back(workspace.stat[stat_m * i + 1]);
    solver_statistics_.sigma.push_back(workspace.stat[stat_m * i + 2]);
    solver_statistics_.alpha_prim.push_back(workspace.stat[stat_m * i + 3]);
    solver_statistics_.alpha_dual.push_back(workspace.stat[stat_m * i + 4]);
    solver_statistics_.mu.push_back(workspace.stat[stat_m * i + 5]);
    solver_statistics_.res_stat.push_back(workspace.stat[stat_m * i + 6]);
    solver_statistics_.res_eq.push_back(workspace.stat[stat_m * i + 7]);
    solver_statistics_.res_ineq.push_back(workspace.stat[stat_m * i + 8]);
    solver_statistics_.res_comp.push_back(workspace.stat[stat_m * i + 9]);
    solver_statistics_.obj.push_back(workspace.stat[stat_m * i + 10]);
    solver_statistics_.lq_fact.push_back(workspace.stat[stat_m * i + 11]);
    solver_statistics_.itref_pred.push_back(workspace.stat[stat_m * i + 12]);
    solver_statistics_.itref_corr.push_back(workspace.stat[stat_m * i + 13]);
    solver_statistics_.lin_res_stat.push_back(workspace.stat[stat_m * i + 14]);
    solver_statistics_.lin_res_eq.push_back(workspace.stat[stat_m * i + 15]);
    solver_statistics_.lin_res_ineq.push_back(workspace.stat[stat_m * i + 16]);
    solver_statistics_.lin_res_comp.push_back(workspace.stat[stat_m * i + 17]);
  }
  if (!solver_statistics_.obj.empty()) {
    solver_statistics_.obj_val = solver_statistics_.obj[solver_statistics_.iter];
  }
...
// HpipmStatus OcpQpIpmSolver::solve(
//     const Eigen::VectorXd& x0, std::vector<OcpQp>& ocp_qp,
...
//   for (int i = 0; i <= dim_.N; ++i) {
//     if (solver_settings_.warm_start) {
//       ACHECK(qp_sol[i].x.size() == dim_.nx[i]) << "qp_sol[" << i << "].x.size() must be " << dim_.nx[i];
//     } else {
...
//   for (int i = 0; i < dim_.N; ++i) {
//     if (solver_settings_.warm_start) {
//       ACHECK(qp_sol[i].u.size() == dim_.nu[i]) << "qp_sol[" << i << "].u.size() must be " << dim_.nu[i];
//     } else {
...
const OcpQpIpmSolverSettings& OcpQpIpmSolver::solver_settings() const {
  return solver_settings_;
}

const OcpQpIpmSolverStatistics& OcpQpIpmSolver::solver_statistics() const {
