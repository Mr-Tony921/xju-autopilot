...
class PathDeciderTest : public ::testing::Test {
 public:
  ...
  std::list<pnc_map::ReferenceLine> GenerateReferenceLines() {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      ref_line.set_max_speed_limit(80.0 / 3.6);
    ...
    ++ref_line;
    ref_line->set_id(std::to_string(2));
  ...
  pnc::TrajectoryPoint PlanningStartPoint() {
    ...
    path_point->set_x(1.874);
    path_point->set_y(20.0);
  ...
  void LoadPlaningConfig() {
    ...
    // std::string lane_decider_file = "/home/ws/src/pnc/configs/planning/lane_decider_default_config.pb.txt";
    pnc::File::GetGflagConfig(gflag_config_file_path);
    pnc::File::GetProtoConfig(planning_config_file, &config_);
    // pnc::File::GetProtoConfig(lane_decider_file, &lane_decider_config_);
  ...
  void LoadInput() {
    std::string prediction_file = "/home/ws/src/pnc/planning/scenarios/common/lane_decider_test/prediction_obstacles.pb.txt";
    std::string chassis_file = "/home/ws/src/pnc/planning/scenarios/common/lane_decider_test/chassis.pb.txt";
    std::string localization_file = "/home/ws/src/pnc/planning/scenarios/common/lane_decider_test/localization.pb.txt";
 ...
 protected:
  ...
  planning::LaneDeciderConfig lane_decider_config_;
...
TEST_F(PathDeciderTest, Process) {
  ADEBUG << "LaneDeciderTest start";
  auto lane_decider_stage = std::make_shared<LaneDecider>(internal_);
  ...
  lane_decider_stage->Process(&frame_);
}
