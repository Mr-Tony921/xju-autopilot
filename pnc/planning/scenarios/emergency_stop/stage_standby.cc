...
EmergencyStopStageStandby::EmergencyStopStageStandby(
    const pnc::StageConfig& config, std::shared_ptr<PlanningInternal>& internal)
    : Stage(config, internal) {}

Stage::Status EmergencyStopStageStandby::Process(Frame* frame) {
  Stage::Process(frame);
...
Stage::Status EmergencyStopStageStandby::FinishStage() {
