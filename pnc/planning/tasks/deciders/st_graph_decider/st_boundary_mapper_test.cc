...
//     if (ptr_obs->is_static()) {
//         ObjectDecisionType stop_decision;
//         stop_decision.mutable_stop();
//         ptr_obs->set_longitudinal_decision(stop_decision);
//     }
...
// TEST_F(StBoundaryMapperTest, IsDangerousStopObs) {
//   PathDecision* path_decision = reference_line_info_.path_decision();
//   std::string id = path_decision->obstacles().Items().back()->id();
//   Obstacle* stop_obs = path_decision->Find(id);
//   StGraphDeciderConfig st_graph_config; 
  
//   StBoundaryMapper st_boundary_mapper(
//       st_graph_config, &reference_line_info_, &frame_);
//   bool ret = st_boundary_mapper.IsDangerousStopObs(stop_obs);
//   EXPECT_TRUE(ret);
// }

// TEST_F(StBoundaryMapperTest, ComputeSTBoundary) {
//   PathDecision* path_decision = reference_line_info_.path_decision();
//   auto obstacles = path_decision->obstacles().Items();
//   std::string id = obstacles.at(1)->id();

//   Obstacle* follow_obs = path_decision->Find(id);
//   StGraphDeciderConfig st_graph_config; 
  
//   StBoundaryMapper st_boundary_mapper(
//       st_graph_config, &reference_line_info_, &frame_);
//   st_boundary_mapper.ComputeSTBoundary(follow_obs);
//   bool ret = follow_obs->st_boundary().IsEmpty();
//   EXPECT_FALSE(ret);
// }

TEST_F(StBoundaryMapperTest, MapObstaclesToSTBoundaries) {

  StGraphDeciderConfig st_graph_config; 
  
  StBoundaryMapper st_boundary_mapper(
      st_graph_config, &reference_line_info_, &frame_);
  bool ret = st_boundary_mapper.MapObstaclesToSTBoundaries();
  AERROR << "MapObstaclesToSTBoundaries"  << ret;
  EXPECT_TRUE(ret);
}
