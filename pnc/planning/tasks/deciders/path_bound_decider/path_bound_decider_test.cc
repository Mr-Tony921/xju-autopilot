...
class PathBoundDeciderTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    LoadPlaningConfig();
    LoadTaskConfig();
    ...
    // frame_.SetPrioritizedReferenceLineInfos(frame_.GetReferenceLineInfos());
    UpdateInternal();
  }

  void UpdateInternal() {
    internal_ = std::make_shared<PlanningInternal>();
    auto planning_status = internal_->mutable_planning_status();
    planning_status->mutable_lateral_shift_status()->set_type(LateralShiftStatus::LEFT);
    planning_status->mutable_lateral_shift_status()->set_distance_m(0.3);
  } 
  ...
  void LoadTaskConfig() {
    for (const auto& default_task_config : config_.default_task_config()) {
      if (default_task_config.task_type()  == pnc::TaskType::PATH_BOUND_DECIDER) {
...
TEST_F(PathBoundDeciderTest, Process) {
  auto path_bound_decider_task = std::make_shared<PathBoundDecider>(task_config_, internal_);
  auto reference_line_info = frame_.GetReferenceLineInfos()->front();
  path_bound_decider_task->Init(task_config_);
  bool status = path_bound_decider_task->Process(&reference_line_info, &frame_);
  EXPECT_TRUE(status);
}
