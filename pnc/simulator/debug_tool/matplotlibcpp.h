...
struct _interpreter {
  // PyObject* s_python_function_use;
  ...
  PyObject* s_python_function_figtext;
  PyObject* s_python_function_ioff;
  PyObject* s_python_function_delaxes;

  /* For now, _interpreter is implemented as a singleton since its currently not
     possible to have
     multiple independent embedded python interpreters without patching the
     python source code
     or starting a separate process for each. [1]
     Furthermore, many python objects expect that they are destructed in the
     same thread as they
     were constructed. [2] So for advanced usage, a `kill()` function is
     provided so that library
     users can manually ensure that the interpreter is constructed and destroyed
     within the
     same thread.
...
// optional but recommended
    ...
    // s_python_function_use = safe_import(matplotlib, "use");
    ...
    s_python_function_figtext = safe_import(pymod, "figtext");
    s_python_function_ioff = safe_import(pymod, "ioff");
    s_python_function_delaxes = safe_import(pymod, "delaxes");
#ifndef WITHOUT_NUMPY
...
inline void figure_size(const std::string& str, size_t w, size_t h) {
  detail::_interpreter::get();
  ...
  PyObject* res = PyObject_Call(
      detail::_interpreter::get().s_python_function_figure,
      /*detail::_interpreter::get().s_python_empty_tuple, */ args, kwargs);
...
inline void figtext(long x, long y, const std::string& str) {
  detail::_interpreter::get();
  ...
  PyObject* res = PyObject_CallObject(
      detail::_interpreter::get().s_python_function_figtext, args);

  if (!res) throw std::runtime_error("Call to figtext() failed.");
...
inline void ioff() {
  ...
  PyObject* res =
      PyObject_CallObject(detail::_interpreter::get().s_python_function_ioff,
                          detail::_interpreter::get().s_python_empty_tuple);
...
inline void delaxes() {
  ...
  PyObject* res =
      PyObject_CallObject(detail::_interpreter::get().s_python_function_delaxes,
                          detail::_interpreter::get().s_python_empty_tuple);
...
// inline void use(const std::string& str){
//     detail::_interpreter::get();
...
//   PyObject* res = PyObject_CallObject(
//       detail::_interpreter::get().s_python_function_use, args);

//   if (!res) throw std::runtime_error("Call to use() failed.");
