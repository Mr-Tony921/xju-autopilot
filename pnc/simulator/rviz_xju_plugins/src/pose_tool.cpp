...
void PoseTool::deactivate() { arrow_->getSceneNode()->setVisible(false); }
...
int PoseTool::processMouseLeftButtonPressed(
...
int PoseTool::processMouseMoved(
    std::pair<bool, Ogre::Vector3> xy_plane_intersection) {
  int flags = 0;
...
double PoseTool::calculateAngle(Ogre::Vector3 start_point,
...
void PoseTool::logPose(std::string designation,
                       geometry_msgs::msg::Point position,
                       geometry_msgs::msg::Quaternion orientation, double angle,
                       std::string frame) {
  RVIZ_COMMON_LOG_INFO_STREAM(
      "Setting " << designation << " pose: Frame:" << frame << ", Position("
